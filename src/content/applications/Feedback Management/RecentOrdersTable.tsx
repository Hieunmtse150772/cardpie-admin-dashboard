...
const RecentOrdersTable = () => {
  ...
  let count = 0
  const [mergeAccountList, setMergeAccountList] = useState([])
  function mergeArrays(arr1, arr2) {
    console.log(arr1.length, '_ ', arr2.length)
    if (arr1.length !== arr2.length) {
      throw new Error("Hai mảng phải có chiều dài bằng nhau để ghép.");
    }
    ...
    setMergeAccountList(mergedArray);
    return mergedArray;
  }
  const dispatch = useAppDispatch();
  const fetchAccountList = async () => {
    ...
    ).then(result => unwrapResult(result)).then((result) => mergeArrays(result?.data?.filter((item) => Number(item.type_of_premium) === 1), FeedBackData))
  }
  useEffect(() => {
    fetchAccountList();
  }, [page, limit])
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="left">
                    ...
                    >
                      <Stack spacing={1}>
                        <Rating name="half-rating-read" defaultValue={account.vote} precision={0.5} readOnly />
                      </Stack>                    </Typography>
                  </TableCell>
                  <SimpleDialog
